...
exports.calculateEarnings = async (req, res) => {
  try {
    const completedOrders = await Order.aggregate([
      ...
      { $group: { _id: null, totalEarnings: { $sum: "$TotalCost" } } },
    ]);

    const completedBookings = await SpaBooking.aggregate([
      { $match: { Status: "Completed" } },
      { $group: { _id: null, totalEarnings: { $sum: "$TotalPrice" } } },
    ]);

    const totalOrderEarnings = completedOrders[0]
      ? completedOrders[0].totalEarnings
      : 0;
    const totalBookingEarnings = completedBookings[0]
      ? completedBookings[0].totalEarnings
      : 0;

    const totalEarnings = totalOrderEarnings + totalBookingEarnings;

    res.json({ totalEarnings, totalOrderEarnings, totalBookingEarnings });
  } catch (error) {
